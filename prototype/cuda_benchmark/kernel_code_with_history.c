...
__global__ void potential_establish(float* prev_phi, float* next_phi, float* step, int* matrix_size )
{
    ...
    float tmp = prev_phi[ijk+l*z_dim*y_dim*x_dim];
    float ro = 1E-17;
    for(int l=0; l <= 500; l++) 
    {
        prev_phi[ijk+l*z_dim*y_dim*x_dim] = tmp;
        ...
        tmp = 1.75E-10 * (
            dif(prev_phi[ip1jk+l*z_dim*y_dim*x_dim], prev_phi[ijk+l*z_dim*y_dim*x_dim], prev_phi[im1jk+l*z_dim*y_dim*x_dim], x_step) +
            dif(prev_phi[ijp1k+l*z_dim*y_dim*x_dim], prev_phi[ijk+l*z_dim*y_dim*x_dim], prev_phi[ijm1k+l*z_dim*y_dim*x_dim], y_step) +
            dif(prev_phi[ijkp1+l*z_dim*y_dim*x_dim], prev_phi[ijk+l*z_dim*y_dim*x_dim], prev_phi[ijkm1+l*z_dim*y_dim*x_dim], z_step) +
