...
def establish_potential(prev_phi, next_phi):
    return (prev_phi, next_phi)
...
def main():
    ...
    while (time < modeling_time):
            ...
            for i in range(1, n[0] - 1):
                for j in range(1, n[1] - 1):
                    for k in range(1, n[2] - 1):
                        # print(chargeGridElectron[i-1][j-1][k-1], chargeGridCarbon[i-1][j-1][k-1], chargeGridHelium[i-1][j-1][k-1])
        ...
        for num in range(positionCarbon.shape[1]):
            ...
            for dim in range(3):
                ...
                E = tensionCorpusculActingCarbon[i][j][k][dim]/Mee
        ...
        # plt.savefig("./picts/carbon_two_coord_{0:04d}.png".format(time))
        # plt.clf()
        # # fig = None
        ...
        for i, ax in zip(inten, [ax1, ax2, ax3]):
            ...
            # ax.plot([k for _,k,_ in i], color='blue')
            # ax.plot([k for _, _, k in i], color='black')
        plt.savefig("./picts/inten/int_time={0:04d}".format(time))
        plt.clf()
        ...
        for k in range(n[2]):
            ...
            plt.colorbar()
            plt.savefig("./picts/phi/phi_time={0:04d}_z={1:02d}".format(time, k))
            plt.clf()
        ...
        for k in range(n[2]):
            ...
            plt.colorbar()
            plt.savefig("./picts/intensity/intensity_time={0:04d}_z={1:02d}".format(time, k))
            plt.clf()
        ...
        for k in range(n[2]):
            # pdb.set_trace()
            plot_tension = np.zeros((n[0], n[1]))
            for i in range(n[0]):
                for j in range(n[1]):
                    plot_tension[i][j] = np.sqrt(np.sum([tension[i][j][k][l]**2 for l in range(3)]))

            plt.contourf(plot_tension.T, cmap=plt.cm.flag)
            plt.colorbar()
            # print(plot_tension)
            # print("./picts/tension/tension_time={0:04d}_z={1:02}".format(time, k))
            plt.savefig("./picts/tension/tension_time={0:04d}_z={1:02d}".format(time, k))
            plt.clf()
    ...
    for pd, p in zip(plot_data, listen_particles):    
        ...
        plt.savefig("./picts/positions_50_50_pn={}".format(p))
        plt.clf()
        ...
        plt.savefig("./picts/speeds_50_50_pn={}.png".format(p))
        plt.clf()
        ...
        plt.savefig("./picts/speeds_50_50_from_time_pn={}.png".format(p))
        plt.clf()
        ...
        plt.savefig("./picts/positions(position)_50_50_pn={}".format(p))

        fig, (ax1) = plt.subplots(1)
        ax1.plot([i[1] for i in pd], [i[2] for i in pd])#, color=colors[p])
        ax1.set_title("z from y particle num={}".format(p))
        plt.savefig("./picts/positions(position)_2_50_50_pn={}".format(p))
        plt.clf()
