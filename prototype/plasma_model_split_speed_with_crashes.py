...
def main():
    ...
    for x, i in zip(xRange[:-1], range(xRange.shape[0] - 1)):
        for y, j in zip(yRange[:-1], range(yRange.shape[0] - 1)):
            for z, k in zip(zRange[:-1], range(zRange.shape[0] - 1)):
                ...
                speed = speed/Mnue
                speed = 0.0
                radiusBigElectron = (hi * radiusElectron ** 3 * cn) ** (1.0 / 3.0) * 10.0**8
                ...
                speed = speed/Mnuc
                speed = 0.0
                radiusBigCarbon = (hi * radiusCarbon ** 3 * cn) ** (1.0 / 3.0) * 10.0**6
    ...
    while (time < modelingTime):
        ...
        for e in range(pe.shape[1]):
            for c in range(pc.shape[1]):
                ...
                if np.sqrt((pe[time][e][0] - pc[time][c][0] )**2 + (pe[time][e][1] - pc[time][c][1] )**2 + (pe[time][e][2] - pc[time][c][2] )**2)  <= pe[time][e][3] + pc[time][c][3]  and \
                    ...
                    crashesElectron += [pe[time][e].copy()]
                    print("++++++++++++ELECTRON+++++++++++++++++++++++")
                    print(pe[time][e])
                    print("++++++++++++++++++++++++++++++++++++")
                    spd1 = [(2 * massElectron * pe[time][e][p]  + pc[time][c][p]* (massCarbon - massElectron))/(massCarbon + massElectron) for p in range(5, 8)]
                    spd2 = [(2 * massCarbon * pc[time][c][p]  + pe[time][e][p]* (massElectron - massCarbon))/(massCarbon + massElectron) for p in range(5, 8)]
        ...
        for h in range(ph.shape[1]):
            for c in range(pc.shape[1]):
                ...
                if np.sqrt((ph[time][h][0] - pc[time][c][0] )**2 + (ph[time][h][1] - pc[time][c][1] )**2 + (ph[time][h][2] - pc[time][c][2] )**2)  <= ph[time][h][3] + pc[time][c][3] and \
                    ...
                    crashesHelium += [ph[time][h][:]]
                    print("+++++++++++++++HELIUM+++++++++++++++++++++")
                    print(ph[time][h][:])
                    print("++++++++++++++++++++++++++++++++++++")
                    spd1 = [(2 * massHelium * ph[time][h][p]  + pc[time][c][p]* (massCarbon - massHelium))/(massCarbon + massHelium) for p in range(5, 8)]
                    spd2 = [(2 * massCarbon * pc[time][c][p]  + ph[time][h][p]* (massHelium - massCarbon))/(massCarbon + massHelium) for p in range(5, 8)]
        ...
        for num in crashesElectron + lastCrashesElectron:
            xBig, yBig, zBig = num[0], num[1], num[2]
            ax.scatter(xBig, yBig, zBig, color='green')  #, s=int( radiusBig*1E+14)+35)
            x2, y2, _ = proj3d.proj_transform(xBig, yBig, zBig, ax.get_proj())
            str_label = '{:5.2f},{:5.2f},{:5.2f}'.format(num[5], num[6], num[7])
